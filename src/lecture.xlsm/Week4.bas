...
Function forwardElimination(matrix_a As Variant, b As Variant) As Variant
  Dim i As Integer, k As Integer, j As Integer, n As Integer
	Dim matrix_a_, b_ As Variant
	Dim a_ik, a_kk As Double
	matrix_a_ = matrix_a: b_ = b
  n = UBound(matrix_a, 1)
  For k = 1 To n - 1
    For i = k + 1 To n
      For j = i To n
				a_ik = matrix_a_(i,k)
				a_kk = matrix_a_(k,k)
        matrix_a_(i, j) = matrix_a_(i, j) - matrix_a_(i, k) * matrix_a_(k, j) / matrix_a_(k, k)
        b_(i) = b_(i) - a_ik * b_(k) / a_kk
      Next j
    Next i
  Next k
	' 返り値をVariant型にすることで配列の中に配列をいれて、二つの要素を返すことが出来る
  forwardElimination = Array(matrix_a_, b_)
End Function
